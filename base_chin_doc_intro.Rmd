...
These steps will be described in detail in section [Calibration Procedures](#Calib).(XXX)
...
1.	Select brood years with an adequate number of CWT recoveries. See [“SouthSoundSurrogateTagWorkup.xlsx”](xxx) for more information.<br>
...
**Adjusting Area 8 sport recoveries**<br>
...
Figure xxx. Age-5 Adjusted Escapements and Age-4 to Age-5 TRS Ratios
...
For more information see ["Calibration_CWT_Inputs; Rnd7_9.30.20"](xxx) tab `ImputeAge5Esc`.
...
After the new base period was first implemented, it was discovered that ELW exploitation rates differed from STL exploitation rates, although STL CWTs were used as a surrogate for ELW. This was due to differing age compositions of the abundances going into FRAM. The original cluster and principal components analysis (McHugh 2015) and the historic ratio of simple ERs were not stratified by age or time-step, although the calibration inputs were.
To meet the primary assumption of the surrogate procedure that STL 'outside' ER equals ELW 'outside' ER, ELW's age composition in FRAM is adjusted to reflect average STL age comp.  
Terminal run size forecasts of ELW are re-distributed to a static age composition using the average STL 2007-2013 age composition from validation runs. 
For more information see [ProposalForELDU_7-10-2019.docx](xxx) and [.xls](xxx).
