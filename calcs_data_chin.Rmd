...
**Length at age is stock specific and is constant from year to year.**  The proportion of an age class that is of legal size in size-limit fisheries is determined using the <a title = "VBGF" href = "https://en.wikipedia.org/wiki/Von_Bertalanffy_function" target="_blank">von Bertalanffy growth function</a>. It is assumed that growth in the current model year is similar to that in the years used to estimate the growth parameters (McHugh et al. 2015). 
...
The starting cohort size in time step 1 is a product of two parameters: (*1*) the base period cohort abundance for stock *s* at age *a* (<span title = "Base period cohort abundance estimate of stock s at age a">$BPCohorts_{s,a}$</span>) and (*2*) a stock and age-specific recruit scaler (<span title = "Stock recruit scaler of stock s at age a">$StockScalers_{s,a}$</span>).  The recruit scaler is an annual model input.
...
where <span title = "Discrete natural mortality rate at age a and time step t">$M_{a,t}$</span> is the discrete natural mortality rate for age *a* fish during time step *t* (Appendix 8).
...
- <span title = "">$FisheryScaler_{f,t}$</span> $\times$ <span title = "Release mortaility rate for sublegal fish in fishery f during time step t">$ShakerReleaseMortalityRate_{f,t}$</span> is the release mortality rate for sublegal fish in fishery *f* and time step *t* (see Appendix 7).
...
Fishery mortality in pre-terminal Chinook fisheries in <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> can also be expressed in terms of adult equivalents (AEQ), which is the number of fish that would have survived to maturity and escaped to spawn in the absence of fishing.  The <span title = "AEQ: Adult equivalent mortalities">AEQ</span> factors adjust for the natural mortality that would have occurred between the time/age the fish were caught and the time/age that they would have matured. All age-5 Chinook in the mature time step and terminal fisheries have an <span title = "AEQ: Adult equivalent mortalities">AEQ</span> factor of 1. The <span title = "AEQ: Adult equivalent mortalities">AEQ</span> factor is calculated using maturation rates from the reference base period <span title = "Maturation rate of stock s at age a during time step t">$(MatRate_{s,a,t})$</span> as:
...
<p>**(15)**<div style = "text-align:center"><span title = "Total terminal fishery mortality for stock s at age a during time step t">$TotTermMort_{s,a,t}$</span> $=$ $\displaystyle\sum_{f \in terminal}$ <span title = "Landed catch of stock s at age a in fishery f during time step t">$(Catch_{s,a,f,t}$</span> $+$ <span title = "Drop-off mortality of stock s at age a in fishery f at time step t">$Dropoff_{s,a,f,t}$</span> $+$ <span title = "Sublegal encounters of stock s at age a in fishery f during time step t">$Shakers_{s,a,f,t}$</span> $+$ <span title = "Adult release mortalities of stock s at age a in fishery f during time step t">$AdultRelMort_{s,a,f,t}$</span> $+$ <span title = "Non-retention mortality of stock s at age a in fishery f during time step t">$CNR_{s,a,f,t})$</span> </div> </p>
...
Escapement is defined as the number of fish that remain in the mature cohort after removal of all terminal-area fishery related mortality.  For Chinook, the <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> escapement represents the ‘run-to-the-river’ as freshwater fisheries are not processed in <span title = "FRAM: Fishery Regulation Assessment Model">FRAM.</span> 
...
Finally, <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> sends the terminal marine and freshwater run sizes and fishing mortalities to the <span title = "TAMM: Terminal Area Management Module">TAMM</span> file for use in additional calculations and final reporting.

# Processing Schematic

To facilitate understanding of the sequencing in <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> processing steps, Table 3 provides an example of how a Chinook stock’s cohort is processed by age and time step from starting cohort to escapement. For brevity, the table only shows ages 3 and 4, but these processing steps occur for all ages (2 to 5).
...
**(17)**
$$ER_s = \frac{\displaystyle\sum\limits_a \displaystyle\sum\limits_{t \! = \! 2}^4 AEQTotMort_{s,a,t}}{\displaystyle\sum\limits_a \displaystyle\sum\limits_{t \! = \! 2}^4 AEQTotMort_{s,a,t} + \displaystyle\sum\limits_{a \! = \! 3}^5 \displaystyle\sum\limits_t Escapement_{s,a,t}}$$

# Size limit evaluations

<span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> uses a <a title = "VBGF" href = "https://en.wikipedia.org/wiki/Von_Bertalanffy_function" target="_blank">von Bertalanffy growth function</a> to estimate the proportion of a stock-at-age that is greater than the minimum size limit in a fishery and time step. As stated above, this proportion is also referred to as the <span title = "PV: Proportion vulnerable">PV</span>. Chinook greater than the minimum size limit are considered to be of “legal size” and thus vulnerable to be caught in a fishery. Conversely, Chinook smaller than the minimum size limit are of “sublegal size” and ineligible to be retained. 
...
- <span title = "Original calculation of landed catch">$OriginalLandedCatch$</span> = Landed catch under base period size limit regulations
- <span title = "New calculation of landed catch">$NewLandedCatch$</span> = Landed catch under new size limit regulations
- <span title = "Original calculation of sublegal encounters">$OriginalSublegalEncounters$</span> = Sublegal encounters under base period size limit regulations
- <span title = "New calculation of sublegal encounters">$NewSublegalEncounters$</span> = Sublegal encounters under new size limit regulations

See (Hagen-Breaux et al. 2013) for a detailed description of the equations.

# Model-stock proportion

While the stocks included in Chinook <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> account for the majority of the Chinook caught in <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> fisheries, there are some stocks that are not represented in the model (see section 2.1). To account for these stocks, fisheries with non-model-stock impacts are supplied with an externally computed model-stock proportion. The Model-stock proportion <span title = "MSP: Model-stock proportion">(MSP)</span> is a constant that is either calculated independently or produced during base period development. It represents the percentage of a fishery’s total catch that is accounted for by model stocks. 

**(20)**
$$MSP_f = \frac{\displaystyle\sum\limits_{s,a,t} Catch_{s,a,f,t}}{TotalCatch_f}$$

# Terminal Area Management Module
...
```{r terminal management regions table, echo = FALSE}
dplyr::tibble(
...
- Receive inputs for <span title = "TAMM: Terminal Area Management Module">TAMM</span> stock abundances 
- Receive input for <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span>-<span title = "TAMM: Terminal Area Management Module">TAMM</span> stock management criteria (updated annually)
...
- Calculate the FRAM fishery scaler that produces the <span title = "Catch calculated in TAMM">$Catch_{TAMM}$</span>:
...
- Rerun FRAM and repeat these steps until <span title = "Catch calculated in TAMM">$Catch_{TAMM}$</span> is within 0.1% (or fewer than 4 fish) of <span title = "Catch calculated in FRAM">$Catch_{FRAM}$</span>.

```{r chinook FRAM TAMM flow chart image, echo=FALSE}
...
To back-calculate starting cohorts given a known terminal run size, the catch summaries entering the main equation have to strictly follow <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> terminal run size definitions and handling (i.e. terminal versus pre-terminal treatment, inclusion or exclusion of incidental mortality from <span title = "TRS: Terminal run size">TRS</span> definitions, etc.).
...
- <span title = "Backwards FRAM escapement or (extreme) terminal run size target">$BkTarget$</span> = Backwards <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span> run size target; escapement or (extreme) terminal run size
- <span title = "Base period starting abundance of a cohort">$BPCohort$</span> = Base period starting cohort
- <span title = "Stock recruit scaler">$StockScaler$</span> = Stock recruit scaler (abundance)
