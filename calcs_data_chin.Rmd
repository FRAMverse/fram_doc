...
Chinook non-retention mortalities are modeled using inputs of legal and sub-legal encounters (*CNR Method 3*), or inputs of total encounters (*CNR Method 4*), or from the level of open versus non-retention effort within each time step (*CNR Method 2*).  The methods were developed to fit observations available from various fisheries. *CNR Method 2*,  which has not been used in recent years, was developed for Canadian and Alaskan fisheries which had both open and non-retention regulation periods in the same season and had changes in the gear or fishing patterns to avoid Chinook encounters.

Most non-retention calculations need the proportion of the legal ($LegalPropTempCatch_{s,a,f,t}$) and sublegal ($SubPropTempCatch_{s,a,f,t}$) catch in a fishery and time step that is comprised of a given stock and age. 

These proportion are computed in three steps.

*Step 1*: Compute the catch by stock, age, fishery and time step as in equations 3 and 5 with a $FisheryScaler$ of 1.
...
*Step 3*: Compute the proportion of the catch made up of a given stock and age.
...
This method requires a flag of "2" in the FRAM user interface as well as four inputs.

- Field value 1: Number of days the non-retention fishery is open within a fishery and time step <u>$CNRDays$</u>$_{f,t}$
- Field value 2: Number of days the retention fishery is open within a fishery and time step <u>$RetentionDays$</u>$_{f,t}$
- Field value 3: Legal selectivity rate <u>$LegalSelRate$</u>$_{f,t}$
- Field value 4: Sublegal selectivity rate <u>$SublegSelRate$</u>$_{f,t}$

*The legal and sublegal selectivity rates provide the modeler with the ability to adjust model output that is based on landed catch; i.e., if the modeler believes that encounters in a retention fishery are twice as high as in a non-retention fishery, the selectivity rate would be 0.5.*

<p>**(11a)** <div style = "text-align:center"> $CNRLegal_{s,a,f,t} = Catch_{s,a,f,t} \times$ $\frac{\underline{CNRDays}_{f,t}}{\underline{RetentionDays}_{f,t}}$ $\times$ <u>$LegalSelRate$</u>$_{f,t} \times$ <u>$LegalReleaseMortalityRate$</u>$_{f,t}$ </div> </p>

<p>**(11b)** <div style = "text-align:center"> $CNRSub_{s,a,f,t} = Shakers_{s,a,f,t} \times$ $\frac{\underline{CNRDays}_{f,t}}{\underline{RetentionDays}_{f,t}}$ $\times$ <u>$SublegSelRate$</u>$_{f,t}$ </div> </p>

<u> CNR Method 3</u> - Computed from external estimate of legal and sub-legal sized encounters
For this method the modeler provides external estimates of legal ($LegalEnc_{f,t}$) and sublegal ($SublegEnc_{f,t}$) fishery encounters. FRAM split encounters into stocks and ages and applies a release mortality.

<p>**(12a)** <div style = "text-align:center"> $CNRLegal_{s,a,f,t} =$ <u>$LegalEnc$</u>$_{f,t} \times LegalPropTempCatch_{s,a,f,t} \times$ <u>$LegalReleaseMortalityRate$</u>$_{f,t} \times$ <span title = "Model-stock proportion of fishery f
Base period calculation"><u>$MSP$</u>$_{f}$ </div> </p>

<p>**(12b)** <div style = "text-align:center"> $CNRSub_{s,a,f,t} =$ <u>$SublegEnc$</u>$_{f,t} \times SublegPropTempCatch_{s,a,f,t} \times$ <u>$ShakerReleaseMortalityRate$</u>$_{f,t} \times$ <span title = "Model-stock proportion of fishery f
Base period calculation"><u>$MSP$</u>$_{f}$ </div> </p>

<u> CNR Method 4</u> - Computed from external estimates of total encounters
For this method the modeler provides external estimates of total fishery encounters ($TotalEstEnc_{f,t}$). FRAM first computes legal and sublegal encounters using a fishery scaler of 1 (see LegalTempCatch and SublegTempCatch equations above). Next, FRAM calculates a CNRScaler to produce total encounters that match the target encounters (TotalEstEnc). After multiplying legal and sublegal encounters with the CNRScaler a release mortality is applied to convert encounters into mortalities.

<p>**(13a)** <div style = "text-align:center"> $CNRScaler_{f,t} = $$\frac{\underline{TotalEstEnc}_{f,t} \times \underline{MSP}_{f}}{\sum_{s} \sum_{a} LegalTempCatch_{s,a,f,t} + \sum_{s} \sum_{a}SublegTempCatch_{s,a,f,t}}$ </div> </p>

<p>**(13b)** <div style = "text-align:center"> $CNRLegal_{s,a,f,t} = LegalTempCatch_{s,a,f,t} \times CNRScaler_{f,t} \times$ <u>$LegalReleaseMortalityRate$</u>$_{f,t}$ </div> </p>

<p>**(13c)** <div style = "text-align:center"> $CNRSub_{s,a,f,t} = SublegTempCatch_{s,a,f,t} \times CNRScaler_{f,t} \times$ <u>$ShakerReleaseMortalityRate$</u>$_{f,t}$ </div> </p>
...
- $LegalPropTempCatch_{s,a,f,t}$ is the proportion of legal catch of stock s, age a, in fishery f, at time step t using FisheryScalar = 1.0 
- $MSP_{f}$ is the fishery model stock proportion as described in chapter 4.6
- $ShakerReleaseMortalityRate_{f,t}$ is the mortality rate of sublegal encounters
- $SublegPropTempCatch_{s,a,f,t}$ is the proportion of sublegal encounters of stock s, age a, in fishery f, at time step t using FishScalar = 1.0
- $SublegTempCatch_{s,a,f,t}$ are the sublegal encounters using FisheryScaler = 1
- $TotalLegalTempCatch_{f,t}$ is the total catch in fishery f at time step t summed over all stocks and ages using FisheryScaler = 1
- $TotalSublegTempCatch_{f,t}$ are the total encounters in fishery f at time step t summed over all stocks and ages using FisheryScaler = 1
...
<p>**(14b)**<div style = "text-align:center"><span title = "Mark-selective fishery catch of stock s at age a in fishery f during time step t">$MSFCatch_{s,a,f,t}$</span> $=$ <span title = "Cohort abundance of stock s at age a during time step t">$Cohort_{s,a,t}$</span> $\times$ <span title = "Base period exploitation rate of stock s at age a in fishery f during time step t.
Base period calculation"><u>$BPER$</u>$_{s,a,f,t}$</span> $\times$ <span title = "Fishery scaler of fishery f at time step t.
...
<p>**(15a) Marked:**<div style = "text-align:center"><span title = "Adult release mortalities of stock s at age a in fishery f during time step t">$AdultRelMort_{s,a,f,t}$</span> $=$ <span title = "Cohort abundance of stock s at age a during time step t">$Cohort_{s,a,t}$</span> $\times$ <span title = "Base period exploitation rate for stock s at age a in fishery f during time step t.
Base period calculation"><u>$BPER$</u>$_{s,a,f,t}$</span> $\times$ <span title = "Fishery scaler of fishery f at time step t.
...
<p>**(15b) UnMarked:**<div style = "text-align:center"><span title = "Adult release mortalities of stock s at age a in fishery f during time step t">$AdultRelMort_{s,a,f,t}$</span> $=$ <span title = "Cohort abundance of stock s at age a during time step t">$Cohort_{s,a,t}$</span> $\times$ <span title = "Base period exploitation rate for stock s at age a in fishery f during time step t.
Base period calculation"><u>$BPER$</u>$_{s,a,f,t}$</span> $\times$ <span title = "Fishery scaler of fishery f at time step t.
...
Base period calculation"><u>$sfm$</u>$_{f,t}$</span> <span title = "Proportion vulnerable of stock s at age a in fishery f during time step t">$\times PV_{s,a,f,t}$</span> <span title = "Stock fishery rate scaler for stock s in fishery f at time step t.
Input by user">$\times$ <b>$SHRF$</b>$_{s,f,t}$</span> </div> </p>
...
<p>**(18a) For time steps 1 and 2:**<div style = "text-align:center"><span title = "Adult equivalent mortalities for stock s at age a during time step t.
...
<p>**(18b) For time step 3 (final base period time step):**<div style = "text-align:center"><span title = "Adult equivalent mortalities for stock s at age a during time step t.
Base period calculation"><u>$AEQ$</u>$_{s,a,t}$</span> $=$ <span title = "Maturation rate of stock s at age a during time step t.
Base period calculation"><u>$MatRate$</u>$_{s,a,t}$</span> $+ \ (1 \ -$ <span title = "Maturation rate of stock s at age a during time step t.
...
<p>**(20)**<div style = "text-align:center"><span title = "Mature cohort abundance of stock s at age a during time step t">$MatureCohort_{s,a,t}$</span> $=$ <span title = "Cohort abundance of stock s at age a during time step t">$Cohort_{s,a,t}$</span> $\times$ <span title = "Maturation rate of stock s at age a during time step t.
...
<p>**(21a)** For time steps 2 and 3:<div style = "text-align:center"><span title = "Cohort abundance of stock s at age a during time step t+1">$Cohort_{s,a,t \! + \! 1}$</span> $=$ <span title = "Cohort abundance of stock s at age a during time step t">$Cohort_{s,a,t}$</span> $-$ <span title = "Mature cohort abundance of stock s at age a during time step t">$MatureCohort_{s,a,t}$</span> </div> </p>

<p>**(21b)** For time step 4:<div style = "text-align:center"><span title = "Cohort abundance of stock s at age a+1 during time step 4">$Cohort_{s,a \! + \! 1,t \! = \! 4}$</span> $=$ <span title = "Cohort abundance of stock s at age a during time step 3">$Cohort_{s,a,t \! = \! 3}$</span> $-$ <span title = "Mature cohort abundance of stock s at age a during time step 3">$MatureCohort_{s,a,t \! = \! 3}$</span> </div> </p>

<p>**(21c)** Except:<div style = "text-align:center"><span title = "Cohort abundance of stock s at age 2 during time step 4">$Cohort_{s,a \! = \! 2,t \! = \! 4}$</span> $=$ <span title = "Cohort abundance of stock s at age 2 during time step 1">$Cohort_{s,a \! = \! 2,t \! = \! 1}$</span> </div> </p>
...
<p>**(25)**<div style = "text-align:center"><span title = "New landed catch calculation of stock s at age a in fishery f during time step t">$NewLandedCatch_{s,a,f,t}$</span> $=$ <span title = "Original landed catch calculation of stock s at age a in fishery f during time step t">$OriginalLandedCatch_{s,a,f,t}$</span> $+$ <span title = "Original calculation of sublegal encounters of stock s at age a in fishery f during time step t">$(OriginalSublegalEncounters_{s,a,f,t}$</span> $-$ <span title = "New calculation of sublegal encounters of stock s at age a in fishery f during time step t">$NewSublegalEncounters_{s,a,f,t})$</span> </div> </p>
...
<p>**(28)**<div style = "text-align:center"><span title = "Stock scaler for stock s at age a during time step 1">$StockScaler_{s,a,t \! = \! 1}$</span> $=$ <span title = "Backwards FRAM target size for stock s at age a.
...
<p>**(29)**<div style = "text-align:center"><span title = "(Extreme) terminal run size estimate from a forward FRAM calculation for stock s at age a">$| \ FRAMescapement_{s,a}$</span> $-$ <span title = "Backwards FRAM escapement or (extreme) terminal run size target of stock s at age a.
