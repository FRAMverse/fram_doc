...
Click "FRAM utilities" from the main menu, and then "Copy model run" to reach the copied recordset screen.^[Note that creating a completely *de novo* "new" blank run is not actually possible from the FRAM application, but would require using other tools to initialize a new .mdb with the necessary lookup and parameter tables.]
...
Next, to edit a mark-selective quota, for Area 10E Sport during the July to September time step 3,
  ...
  - change the input value in Quota (ensure that the flag = 8 for MSF Quota) by clicking twice on the cell, modifying the value, and clicking any other cell in the interface to move “off” your changed cell.
