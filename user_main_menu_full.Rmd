...
This message can be triggered if an integer size exceeds 32 bits; i.e., catches calculated in FRAM are too large. The underlying problem can be a catch input that is too large or mis-flagged in TAMM. Check the modeled catches in <span title = "FRAM: Fishery Regulation Assessment Model">FRAM</span>’s user interface for unusually large values. Check the PopStat report for stocks with negative escapements. Also check the “Detail” of the error message to find out which procedure triggers the message. If it is a TAMM procedure, then error check TAMI (catches flagged as rates, etc.).
