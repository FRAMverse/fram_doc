...
## Estimating Sublegal Mortalities - Galen

## Estimating Non-Retention Mortalities

## Calculating Exploitation Rates (Forward Cohort Reconstruction)
...
(xxx)
```{r OOBProc, echo = FALSE, fig.cap=c("Figure xxx Out-of-Base Procedures"),fig.pos='H', out.height = "800px", out.width='800px'}
...
These encounters are then split into ages by calculating the proportion of stock/age that is of sublegal size and multiplying times the cohort size of the stock age.
$$Sublegals_{a} = Cohort_{a} * SubLegProp_{a} $$
(xxx)
Where the proportion sublegal (SubLegProp) is calculated as in chapter <a href="https://framverse.github.io/fram_doc/calcs_data_chin.html#45_Size_Limit_Evaluations"> Size Limit Evaluations </a> (xxx)
When summed over all ages, the age proportions are then calculated as:
$$Proportion_{a} = \frac{Sublegals_a}{\sum_a Sublegals_a}$$
(xxx)
Sublegal mortalities are then calculated as:
$$Shakers_{s,a,f,t} = Encounters_{s,f,t} * Proportion_a * ShakerReleaseMortalityRate_{f,t}$$ 
(xxx)

- <a href="https://framverse.github.io/fram_doc/base_chin_doc_calibration.html#Backwards Cohort Reconstruction"> Calculate Cohort </a>(xxx)

- Check Convergence
Steps 2 and 3 are iterative, because sublegal mortalities are a function of the cohort size and the cohort size is dependent on sublegal mortalities. These steps are repeated until age-2 abundances converge. This occurs when abundances differ by less than 1%.
...
- Adjust OOB Exploitation Rates to New Base Period Exploitation Rates and Calculate Catches and Escapements
    + Calculate adjusted for base period effort exploitation rates 

The program adjusts exploitation rates for a fishery and time step if the summed
