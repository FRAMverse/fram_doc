...
The current calibration only uses the MSP calculation as described in equationxxx for a few fisheries, because of the high variance associated with estimating the parameters entering the calculation of CWTCatch~f,t~. When flagged as "2", if the MSP exceeds 100% of the BPObsCatch~f~, the program scales CWTCatch~s,a,f,t~ to sum to 1.
...
A second expansion, the brood year expansion, is applied within FRAMBuilder. The default method gives equal weight to all brood years within the model, but users can also specify brood year weights. The brood year expansion is specific to individual brood years and applied to every tag within a given brood year.  To calculate the expansion rate, the total number of sampling expanded tags for each brood year should be summed. The total sum of sampling expanded tags within the brood year with the largest number of recoveries is then divided by the total sum of sampling expanded tags within the brood year of interest. For our example tag, the table below demonstrates that the brood year expansion applied should be 1.02. The brood year expanded value of the example recovery becomes 7.11 (6.97 * 1.02).
...
After the expansion is applied, tags recovered for a given stock, age, fishery, time step are summed across brood years. At this point, three additional tags, including two from the 2007 brood year and one from the 2006 brood year are added to the example tag, resulting in a merged expanded value of **16.6**. For additional information about the brood year expansion and merging process, please refer to the FRAMBuilder sectionxxx of the base period documentation.
